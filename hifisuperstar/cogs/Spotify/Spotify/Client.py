...
class Client:
    ...
    def get_playlist(self, interaction, birdy_uri):
        if self.get_birdy_uri_type(birdy_uri) != "playlist":
        ...
        if playlist_id is None:
            error(
                self,
                f"Spotify: Given birdy URI '{birdy_uri}' does not contain a playlist ID!",
            )
            return None
        ...
        try:
            ...
            if not spotify_playlist:
                error(
                    self,
                    f"Spotify: Unable to fetch playlist information for Spotify playlist ID '{playlist_id}'!",
                )
                return None

            playlist = Playlist(interaction.guild.id)
            ...
            error(self, f"Spotify: Exception occurred - {e!s}")
            return None
    ...
    def get_birdy_uri(self, spotify_url):
        ...
        parsed_path = (
            parsed_url.path[1 : len(parsed_url.path)].split("/")
            if len(parsed_url) > 0
            else []
        )
