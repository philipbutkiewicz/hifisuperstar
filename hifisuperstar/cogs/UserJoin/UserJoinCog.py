...
class UserJoinCog(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if "enabled" not in userjoin:
            warn(
                self,
                "Event warning: User join cog has not been configured",
                member.guild,
            )
            return False
        ...
        if "channel" not in userjoin:
            error(
                self, "Event error: Channel name has not been configured", member.guild
            )
            return False

        if not userjoin["enabled"]:
            warn(
                self, "Event warning: User join cog has not been enabled", member.guild
            )
            return False
    ...
    @app_commands.command(
        name="set_userjoin_channel",
        description="Configures the channel to send messages to",
    )
    @app_commands.checks.has_role("Admin")
    async def set_userjoin_channel(
        ...
        save_resource(
            "userjoin", interaction.guild.id, {"channel": channel, "enabled": True}
        )

        await respond(interaction, f"Channel {channel} has been set.")

    @app_commands.command(
        name="disable_userjoin", description="Disables the user join cog"
    )
    @app_commands.checks.has_role("Admin")
    async def disable_userjoin(self, interaction: discord.Interaction):
        ...
        save_resource(
            "userjoin", interaction.guild.id, {"channel": None, "enabled": False}
        )
