...
class ImageSearchCog(commands.Cog):
    ...
    @app_commands.command(
        name="image_search_config",
        description="Displays the current image search config",
    )
    ...
    async def image_search(
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.IMAGE_SEARCH, interaction
        ):
            return False

        info(
            self,
            f"Image search request: '{query}' {item_index}/{max_items}",
            interaction.guild,
        )
        ...
        if (
            ...
            or item_index + (max_items - 1) > len(images) - 1
        ):
            warn(self, "Invalid input", interaction.guild)
            return await respond(
                interaction,
                f"ERROR: Index out of range, max index is {len(images) - 1}",
            )

        await respond(
            interaction,
            f"{len(images)} results for '{query}', displaying index {item_index}",
        )
        for i in range(item_index, item_index + max_items):
            await respond(interaction, f"{images[i]['title']}\n{images[i]['image']}")

    @app_commands.command(
        name="random_image_search", description="Conducts a random image search"
    )
