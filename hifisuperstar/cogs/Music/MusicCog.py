...
def resolve_track_cache_url(track_url: str) -> str:
    ...
    with YoutubeDL(
        {"format": "m4a/bestaudio/best", "noplaylist": True, "quiet": True}
    ) as ydl:
        yt_info = ydl.extract_info(f"ytsearch:{track_url}", download=False)["entries"][
            0
        ]
...
def upload_zip_to_s3(
    s3_config: dict, key: str, data: bytes, progress_callback=None
) -> str:
...
def list_playlist_packages(s3_config: dict, guild_id: str) -> list:
    ...
    response = client.list_objects_v2(
        Bucket=s3_config["Bucket"], Prefix=f"playlists/{guild_id}/"
    )
...
class PlaylistPageView(discord.ui.View):
    def __init__(
        self,
        playlist: Playlist,
        per_page: int = PLAYLIST_TRACKS_PER_PAGE,
        timeout: float = 120,
    ):
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(
            text=f"Page {self.page + 1}/{self.max_page + 1} — {len(tracks)} track(s) total"
        )
...
class PlaylistDownloadsView(discord.ui.View):
    def __init__(
        ...
        s3_config: dict,
        packages: list,
        per_page: int = PLAYLIST_DOWNLOADS_PER_PAGE,
        timeout: float = 120,
    ):
        super().__init__(timeout=timeout)
    ...
    def _build_select(self):
        for item in [
            child for child in self.children if isinstance(child, discord.ui.Select)
        ]:
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        try:
            url = await asyncio.to_thread(
                get_playlist_package_download_url, self.s3_config, key
            )
        except Exception as e:
            return await interaction.response.send_message(
                f"ERROR: Failed to generate a download link ({e})", ephemeral=True
            )

        link_view = discord.ui.View()
        link_view.add_item(
            discord.ui.Button(
                label="Download", style=discord.ButtonStyle.link, url=url, emoji="⬇️"
            )
        )
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description="\n".join(lines)
            if lines
            else "No packages have been generated yet.",
            color=discord.Color.blurple(),
        )
        embed.set_footer(
            text=f"Page {self.page + 1}/{self.max_page + 1} — {len(self.packages)} package(s) total"
        )
...
class QueuePageView(discord.ui.View):
    def __init__(
        self, player, per_page: int = PLAYLIST_TRACKS_PER_PAGE, timeout: float = 120
    ):
        super().__init__(timeout=timeout)
        self.player = player
        self.per_page = per_page
    ...
    def _build_select(self):
        ...
        for i, track in enumerate(tracks, start=start):
            is_current = (
                current_track is not None and current_track["id"] == track["id"]
            )
            select.add_option(
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(
            text=f"Page {self.page + 1}/{self.max_page + 1} — {total} track(s) total"
        )
...
class SearchResultsView(discord.ui.View):
    ...
    def _build_select(self):
        ...
        for i, entry in enumerate(items, start=start):
            ...
            description_parts = [
                part
                for part in (uploader, format_duration(duration) if duration else None)
                if part
            ]
            select.add_option(
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=f"🔎 Selected '{entry.get('title', video_url)}'!",
            embed=None,
            view=self,
        )
    ...
    def build_embed(self) -> discord.Embed:
        ...
        embed.set_footer(
            text=f"Page {self.page + 1}/{self.max_page + 1} — {len(self.results)} result(s)"
        )
...
class MusicCog(commands.Cog):
    ...
    @app_commands.command(
        name="play",
        description="Plays or queues media, YouTube links, YouTube search queries and direct media "
        "links are supported.",
    )
    async def play(self, interaction: discord.Interaction, query: str | None = None):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_PLAY, interaction
        ):
            return False

        info(
            self,
            f"Playback request{': ' + query if not query is None else ''}",
            interaction.guild,
        )
        ...
        if not track_info:
            ...
            return await respond(
                interaction,
                f"ERROR: Failed to queue '{'the current playlist' if query is None else query}'",
            )

        await respond(
            interaction,
            f"Queued '{track_info['title'] if not query is None else 'the current playlist'}'!",
        )

    @app_commands.command(
        name="search",
        description="Searches YouTube and lets you pick a track from a list to play",
    )
    async def search(self, interaction: discord.Interaction, query: str):
        ...
        try:
            results = await asyncio.to_thread(
                media_search_youtube, query, SEARCH_RESULTS_LIMIT
            )
        except Exception as e:
    ...
    async def play_radio(self, interaction: discord.Interaction, query: str | None = None):
        ...
        info(
            self,
            f"Radio playback request{': ' + query if not query is None else ''}",
            interaction.guild,
        )
    ...
    @app_commands.command(
        name="playlist",
        description="This command can queue a YouTube playlist if supplied with a valid URL, "
    ...
    async def playlist(self, interaction: discord.Interaction, query: str):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_PLAYLIST_QUEUE, interaction
        ):
            return False
        ...
        if not validators.url(query) and not re.match(allowed_chars_regex, query):
            warn(self, "Invalid input", interaction.guild)
            return await respond(
                interaction,
                "ERROR: Invalid format, only alphanumeric characters or URLs are allowed.",
            )
    ...
    @app_commands.command(
        name="list_playlists", description="Lists available playlists"
    )
    async def list_playlists(self, interaction: discord.Interaction):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_PLAYLIST_LIST, interaction
        ):
            return False
        ...
        embed = discord.Embed(
            ...
            description="\n".join(
                f"`{i + 1}.` {name}" for i, name in enumerate(playlists)
            ),
            color=discord.Color.blurple(),
        )
    ...
    @app_commands.command(
        name="playlist_view", description="Displays the contents of a saved playlist"
    )
    async def playlist_view(self, interaction: discord.Interaction, name: str):
        ...
        if not validators.url(name) and not re.match(allowed_chars_regex, name):
            warn(self, "Invalid input", interaction.guild)
            return await respond(
                interaction,
                "ERROR: Invalid format, only alphanumeric characters or URLs are allowed.",
    ...
    async def _resolve_and_cache_track(self, track, interaction):
        ...
        try:
            ...
            warn(
                self,
                f"Track '{track['title']}' could not be resolved ({e}), skipping",
                interaction.guild,
            )
            return None
        ...
        if not is_youtube_url(cache_url):
            warn(
                self,
                f"Track '{track['title']}' is not a YouTube source and can't be cached, skipping",
                interaction.guild,
            )
            return None
        ...
        if not os.path.exists(track_path):
            info(
                self,
                f"Track '{track['title']}' not cached yet, fetching...",
                interaction.guild,
            )
            try:
                ...
                warn(
                    self,
                    f"Track '{track['title']}' could not be fetched ({e}), skipping",
                    interaction.guild,
                )
                return None
        ...
        if not os.path.exists(track_path):
            warn(
                self,
                f"Track '{track['title']}' still missing from cache after fetching, skipping",
                interaction.guild,
            )
            return None
    ...
    def _make_upload_progress_callback(
        self, interaction: discord.Interaction, label: str, total_bytes: int
    ):
        ...
        def on_progress(bytes_amount):
            with lock:
                ...
                percent = (
                    int((state["uploaded"] / total_bytes) * 100) if total_bytes else 100
                )
            ...
            async def _update():
                try:
                    await interaction.edit_original_response(
                        content=f"⬆️ Uploading '{label}': {step}%..."
                    )
                except discord.HTTPException:
                    pass
    ...
    @app_commands.command(
        ...
        description="Downloads a saved playlist as a zip file of cached tracks",
    )
    async def playlist_download(self, interaction: discord.Interaction, name: str):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_PLAYLIST_DOWNLOAD, interaction
        ):
            return False
        ...
        if not s3_config.get("Enabled"):
            ...
            return await respond(
                interaction,
                "ERROR: Playlist downloads are not configured, ask an admin to set up S3 storage.",
            )
        ...
        try:
            await respond(
                interaction,
                f"⏳ Packing '{playlist.get_name()}': 0/{len(tracks)} tracks processed...",
            )
            ...
            if track_index == 0:
                ...
                return await respond(
                    interaction,
                    f"ERROR: None of the tracks in '{name}' could be fetched or cached.",
                )
            ...
            progress_callback = self._make_upload_progress_callback(
                interaction, playlist.get_name(), len(zip_data)
            )
            try:
                download_url = await asyncio.to_thread(
                    upload_zip_to_s3, s3_config, object_key, zip_data, progress_callback
                )
            except Exception as e:
                error(
                    self,
                    f"Failed to upload playlist zip to S3 ({e})",
                    interaction.guild,
                )
                return await respond(
                    interaction, "ERROR: Failed to upload the playlist zip to storage."
                )

            await respond(
                interaction,
                f"✅ Here's '{playlist.get_name()}' ({track_index} track(s))!\n{download_url}",
            )
        finally:
            self.playlist_downloads_in_progress.discard(guild_id)

    @app_commands.command(
        name="playlist_downloads",
        description="Lists previously generated playlist zip packages",
    )
    async def playlist_downloads(self, interaction: discord.Interaction):
        ...
        try:
            packages = await asyncio.to_thread(
                list_playlist_packages, s3_config, str(interaction.guild.id)
            )
        except Exception as e:
            error(self, f"Failed to list playlist packages ({e})", interaction.guild)
            return await respond(
                interaction, "ERROR: Failed to list playlist packages from storage."
            )

        if not packages:
            return await respond(
                interaction, "No playlist packages have been generated yet."
            )
    ...
    # noinspection DuplicatedCode
    @app_commands.command(
        name="playlist_save", description="Saves the current playlist"
    )
    async def playlist_save(
        self, interaction: discord.Interaction, name: str, overwrite: bool = False
    ):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_PLAYLIST_SAVE, interaction
        ):
            return False
        ...
        if not playlist.save(overwrite=overwrite):
            ...
            return await respond(
                interaction,
                f"ERROR: Failed to save playlist '{name}'"
    ...
    @app_commands.command(
        name="playlist_delete", description="Deletes a playlist with a given name"
    )
    ...
    async def playlist_delete(self, interaction: discord.Interaction, name: str):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_PLAYLIST_DELETE, interaction
        ):
            return False
        ...
        if not playlist.delete():
            ...
            return await respond(
                interaction, f"ERROR: Failed to delete playlist '{name}'"
            )
    ...
    @app_commands.command(
        name="skip", description="Skips current item in the playback queue"
    )
    ...
    @app_commands.command(
        name="jump_to_index", description="Skips to a specific index in the queue"
    )
    async def jump_to_index(self, interaction: discord.Interaction, index: str):
        ...
        if index < 1 or index > len(playlist.get_tracks()):
            ...
            await respond(
                interaction,
                f"ERROR: Invalid format, provide a value between 1 and {len(playlist.get_tracks())}",
            )
            return False
    ...
    @app_commands.command(
        name="queue", description="Lists all items in the playback queue"
    )
    ...
    @app_commands.command(
        name="top_tracks",
        description="Shows the most played tracks (in descending order)",
    )
    async def top_tracks(self, interaction: discord.Interaction):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_TOP_TRACKS, interaction
        ):
            return False
        ...
        display_num_top_tracks = min(display_num_top_tracks, len(tracks))

        lines = []
        for i in range(display_num_top_tracks):
            track = tracks[tracks_keys[i]]
            url = track.get("url")
            track_label = f"[{track['title']}]({url})" if url else track["title"]
            lines.append(
                f"`{i + 1}.` {track_label} — **{track['count']}** play{'s' if track['count'] != 1 else ''}"
            )

        embed = discord.Embed(
            title="Top Tracks", description="\n".join(lines), color=discord.Color.gold()
        )
    ...
    @app_commands.command(
        name="volume",
        description="Sets playback volume to a provided value between 0 and 1.0",
    )
    async def volume(self, interaction: discord.Interaction, volume: str):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_SET_VOLUME, interaction
        ):
            return False
        ...
        try:
            ...
            warn(self, "Invalid input", interaction.guild)
            return await respond(
                interaction, "ERROR: Invalid format, not a float number"
            )

        if volume < 0 or volume > 1:
            warn(self, "Invalid input", interaction.guild)
            return await respond(
                interaction, "ERROR: Invalid format, provide a value between 0 and 1.0"
            )
    ...
    async def repeat(self, interaction: discord.Interaction):
        ...
        if not await Acl(interaction.guild.id).check_and_fail(
            Rule.MUSIC_SET_REPEAT, interaction
        ):
            return False
    ...
    @app_commands.command(
        name="repeat_all", description="Toggles repeat all mode on or off"
    )
