...
class RegexCog(commands.Cog):
        ...
        for key in responses:
            if re.match(responses[key]["match"], message.content):
                ...
                await message.channel.send(
                    responses[key]["response"], reference=message
                )

    @app_commands.command(
        name="set_regex_response", description="Sets an regex response"
    )
    @app_commands.checks.has_role("Admin")
    async def set_regex_response(
        self, interaction: discord.Interaction, match: str, response: str
    ):
        ...
        responses[str_rand_crc32()] = {"match": match, "response": response}
    ...
    @app_commands.command(
        name="delete_regex_response", description="Deletes an regex response"
    )
    @app_commands.checks.has_role("Admin")
    async def delete_regex_response(
    ...
    @app_commands.command(
        name="list_regex_responses", description="Lists all regex responses"
    )
